...
type LiveAnswer = { answer?: string; text?: string; capturedAt?: string; source?: string };
...
export default function Demo() {
  ...
  const unsafe = /\b(he|him|his|she|her|hers)\b/i.test(liveText);
...
function AnswerCard({ kind, source, answer, foot, unsafe = false }: { kind: "baseline" | "kith"; source: string; answer: string; foot: string; unsafe?: boolean }) {
  return <div className={`answer-card answer-card--${kind} ${unsafe ? "answer-card--unsafe" : ""}`}><div className="answer-top"><span className="answer-label">{kind === "kith" && <PresencePulse tone="mint" />}{source}</span><span className={`answer-chip ${kind === "kith" ? "answer-chip--warm" : ""}`}>{kind === "kith" ? "remembered" : "example"}</span></div>{unsafe ? <div className="gentle-pause"><ShieldCheck size={19} /><div><strong>Wait—this doesn’t feel right.</strong><span>The cached answer contains a gendered pronoun. Refresh before using it on camera.</span></div></div> : <p>{answer}</p>}<span className="answer-foot">{foot}</span></div>;
}
